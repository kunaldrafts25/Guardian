...
import 'package:guardian/features/fakecall/presentation/screens/fake_call_screen.dart';
...
class QuickActionsScreen extends ConsumerWidget {
  ...
  Widget _buildCheckInBanner(BuildContext context, WidgetRef ref, CheckInState state) {
    return Container(
      padding: const EdgeInsets.all(16),
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: () => ref.read(checkInProvider.notifier).cancelTimer(),
  ...
  Widget _buildFakeCallSection(BuildContext context) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          children: [
            // Preset callers
            ...FakeCaller.presets.map((caller) => ListTile(
              leading: CircleAvatar(
                backgroundColor: AppColors.secondary.withOpacity(0.2),
                child: Text(caller.name[0]),
              ),
              title: Text(caller.name),
              subtitle: Text(caller.number),
              trailing: PopupMenuButton<Duration>(
                ...
                onSelected: (delay) {
                  FakeCallService.scheduleCall(
                    ...
                    onRing: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) => FakeCallScreen(caller: caller),
                        ),
                      );
                    },
                  );
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Call from ${caller.name} in ${FakeCallService.formatDelay(delay)}'),
                    ),
                  );
                },
                itemBuilder: (_) => FakeCallService.delayOptions.map((delay) => 
                  PopupMenuItem(
                    value: delay,
                    child: Text('In ${FakeCallService.formatDelay(delay)}'),
                  ),
                ).toList(),
              ),
            )),
  ...
  Widget _buildCheckInSection(BuildContext context, WidgetRef ref, CheckInState state) {
    if (state.isActive) {
      return Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
            ...
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: OutlinedButton(
                      onPressed: () => ref.read(checkInProvider.notifier)
        ...
        child: Column(
          children: [
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: CheckInPresets.durations.map((duration) => 
                ActionChip(
  ...
  Widget _buildShakeSection(BuildContext context) {
    return Card(
      child: ListTile(
        ...
        trailing: Switch(
          value: true, // TODO: Connect to settings
          onChanged: (value) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Shake SOS ${value ? "enabled" : "disabled"}'),
              ),
            );
          },
