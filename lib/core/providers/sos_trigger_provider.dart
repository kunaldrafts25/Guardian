...
class SosTriggerNotifier extends StateNotifier<SosTriggerState> {
  ...
  /// Initialize shake detection based on settings
  ...
  void startShakeDetection() {
    if (kIsWeb) return;
    if (state.shakeDetectionActive) return;
  ...
  void _onShakeDetected() {
    ...
    if (emergencyState.isActive) {
      Logger.info('SOS already active, ignoring shake');
      return;
    }

    // Check if countdown is in progress
    if (emergencyState.state == SosState.countdown) {
      Logger.info('Countdown already in progress, ignoring shake');
      return;
    }

    // Trigger SOS from shake source
