...
- Google sign-in through Cognito managed login (authorization-code + PKCE), followed by Guardian device-session enforcement.
...
- Google Maps SDK rendering plus Guardian-authenticated Google Places/Routes proxy endpoints; Guardian remains the emergency/location authority.
...
| Local durability | Drift + ordinary SQLite in OS-protected app storage; see ADR 0009 for the encryption boundary |
...
| Maps and location | Google Maps SDK, Guardian-proxied Google Places/Routes, geolocator |
| API | FastAPI on AWS Lambda through Mangum and API Gateway |
| Identity | Google federation through Amazon Cognito managed login + Guardian device sessions |
...
Production authentication also requires the Cognito managed-login domain generated by the deployed stack:

```text
COGNITO_AUTH_DOMAIN=https://guardian-<environment>-<account>.auth.<region>.amazoncognito.com
COGNITO_CLIENT_ID=<public Cognito mobile client id>
```

Google Maps rendering uses platform-restricted SDK keys. Do not use the server-side
Places/Routes key in the mobile application:

```text
Android environment/local property:
GUARDIAN_GOOGLE_MAPS_ANDROID_API_KEY=<Android-restricted Maps SDK key>

iOS Runner/Config.xcconfig:
GOOGLE_MAPS_IOS_API_KEY=<iOS bundle-restricted Maps SDK key>
```

The backend Places/Routes credential is supplied to the SAM stack as
`GoogleMapsServerApiKeySecretArn` and remains in AWS Secrets Manager.
...
| `GoogleClientId` / `GoogleClientSecret` | Google OAuth web client used only by Cognito federation |
| `GuardianAuthCallbackUrl` / `GuardianAuthLogoutUrl` | Registered Cognito managed-login mobile callbacks |
| `GoogleMapsServerApiKeySecretArn` | Secrets Manager ARN containing a server-restricted Places/Routes key |
| `IncidentRetentionDays` | TTL-backed cloud incident/evidence retention window |
...
2. Configure FCM HTTP v1, APNs sandbox/production, Cognito Google federation, restricted Google Maps Platform keys, Bedrock, and any approved emergency-SMS provider access.
...
8. Monitor notification failures, authentication/session failures, incident persistence, authorization denials, Maps/Routes quota failures, and crash/ANR rates during gradual rollout.
